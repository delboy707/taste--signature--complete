...
function generateQEPImportTemplate() {
  ...
  var q = '"';
  var instRow = instructions.map(function(c) { return q + c.replace(/"/g, q+q) + q; });
  csv += instRow.join(",") + "\n";
  csv += sample.join(",") + "\n";
...
function parseQEPImportCSV(csvText) {
  ...
  rows.forEach(function(row, rowIdx) {
    ...
    colMap.forEach(function(col, i) {
      ...
      if (isNaN(n)) { warnings.push("Row " + (rowIdx + 4) + " col " + col.header + ": not a number"); return; }
