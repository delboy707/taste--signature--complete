...
qep-capture migration 0037, applied to dev and prod), deep links
(`?project=&version=`, surviving the Clerk gate via a sessionStorage stash -
see targets-loaded-ui.js), and "Start Full Evaluation from this target".
`ENABLE_TARGETS_LOADED` is `true`. The qep-capture client waits for ClerkJS
and never sends a tss_shared call as anon (qep-capture-client.js).

Full Evaluation from a target NEVER sets slider values. Targets are shown as
blue markers on their sliders ("Target N", tooltip "Target from <brief> vN");
each stage shows the brief's text as read-only "Brief says:" at its top; the
banner is one line. Sliders stay untouched, so they save as null unless the
